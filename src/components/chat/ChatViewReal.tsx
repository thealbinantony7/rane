...
export function ChatViewReal({
  ...
  return <div className="flex-1 flex flex-col bg-background min-w-0 relative">
    ...
    })) as any} onNavigateToMessage={handleNavigateToMessage} />
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-4 space-y-3">
        {loading ? <div className="flex items-center justify-center py-12">
          ...
          return <div key={message.id} id={`message-${message.id}`}>
            ...
            }} isOwnMessage={isOwnMessage} showAvatar={showAvatar} onReply={() => setReplyingTo(message)} onReact={(msgId, emoji) => handleReact(msgId, emoji)} onBookmark={msgId => handleBookmark(msgId)} replyToMessage={replyToMessage ? {
      ...
      {/* Reply Preview */}
      <AnimatePresence>
        {replyingTo && <ReplyPreview message={{
        id: replyingTo.id,
      ...
      {/* Message Input */}
      {!showVoiceRecorder && <MessageInput onSendMessage={handleSendMessage} onStartVoiceRecording={() => setShowVoiceRecorder(true)} onOpenMediaUpload={() => setShowMediaUpload(true)} selfDestructTimer={selfDestructTimer} onOpenTimerSettings={() => setShowTimerSettings(true)} />}
      ...
      <AISummary isOpen={showAISummary} onClose={() => setShowAISummary(false)} conversationName={getConversationName()} messages={messages.map(m => {
